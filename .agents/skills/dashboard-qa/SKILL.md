...
From the repository root, read `docs/agent-workflows/dashboard-qa.md` completely and follow it. Test only the local dev server with synthetic `?fixture=<name>` scenarios. Report findings; change no source, mutate no application state, and never reach a live or authenticated surface.
